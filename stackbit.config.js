...
const getDocumentLocale = (document) => {
    if (document.fields.slug) {
        return LOCALES.find((locale) => document.fields.slug?.value?.toLowerCase().startsWith(locale.toLowerCase()) || document.fields.slug?.value?.toLowerCase().startsWith('/' + locale.toLowerCase) );
    }
...
class MyContentSource extends ContentfulContentSource {
    ...
    async createDocument(options) {
        if (LOCALIZED_MODELS.includes(options.model.name)) {
            ...
            if (slugField) {
                if (options.updateOperationFields?.slug && !options.updateOperationFields?.slug?.value.toLowerCase().startsWith(options.locale.toLowerCase())) {
